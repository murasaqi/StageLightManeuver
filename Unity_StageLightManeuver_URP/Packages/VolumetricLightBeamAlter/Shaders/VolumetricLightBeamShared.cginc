...
half4 fragShared(v2f i, float outsideBeam)
{
...
#elif VLB_OCCLUSION_DEPTH_TEXTURE
    {
        ...
        float3 depthProps = VLB_GET_PROP(_DynamicOcclusionDepthProps);
        float2 coneMeshProps = VLB_GET_PROP(_ConeGeomProps);

        float2 srcUvCentered = normalize(i.posObjectSpace.xy);
        ...
        float apexDist = coneMeshProps.x;
        ...
        intensity *= lerp(factor, 1, isEqualOrGreater(dynamicOcclusionDepthRaw, 1));
        // only apply factor if(dynamicOcclusionDepthRaw < 1), meaning there is an occlusion
    }
...
#else
    half4 color = i.color;
    color *= tex2D(_GoboTexture, normalize(i.posObjectSpace.xy));
    // color = float4(i.posObjectSpace.x,i.posObjectSpace.y,0,1);
#endif
